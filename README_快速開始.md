...
把整個資料夾部署到任何支援 Node.js 的雲端服務，啟動指令設為：

```bash
npm start
```

服務會自動讀取雲端平台提供的 `PORT`。HTTPS 網址最適合手機動作感測。
...
```text
fire-passing-project/
├─ server.js                 無第三方套件的 Node.js 房間與傳火伺服器
├─ package.json
├─ Dockerfile
├─ render.yaml               可供支援 Blueprint 的平台參考
├─ 啟動傳火計畫.bat
├─ 啟動傳火計畫.command
└─ public/
   ├─ index.html
