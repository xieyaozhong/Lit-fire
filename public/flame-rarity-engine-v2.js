...
(() => {
  if (window.__flameRarityEngineV2Loaded) return;
  window.__flameRarityEngineV2Loaded = true;
  ...
  function renderPreview() {
    ...
    if (ignitionText && !locked) {
      ignitionText.innerHTML = holdPointerId !== null ? '持續<br>按住' : taps.length ? `${taps.length} 下<br>等待成火` : '點擊<br>點火';
    }
  }

  function isShort(values, min, max, ratio) {
    return values.length && values.every((value) => value >= min && value <= max) && spread(values) <= ratio;
  }
  ...
  function matchRainbow(v) {
    return v.length === 3 && v[0] >= 470 && v[0] <= 920 && v[1] <= v[0] * .82 && v[2] <= v[1] * .78 && v[2] >= 70;
  }

  function matchFrost(v) {
    return v.length === 3 && v[0] >= 70 && v[0] <= 270 && v[1] >= v[0] * 1.22 && v[2] >= v[1] * 1.28 && v[2] <= 920;
  }

  function matchThunder(v) {
    return v.length === 4 && isShort([v[0], v[2]], 65, 235, 1.55) && isPause([v[1], v[3]], 420, 850, 1.45) && mean([v[1], v[3]]) >= mean([v[0], v[2]]) * 2.5;
  }

  function matchTide(v) {
    if (v.length !== 5) return false;
    const [a, b, c, d, e] = v;
    return a >= 480 && a <= 900 && a > b * 1.18 && b > c * 1.35 && c >= 70 && c <= 260 && d > c * 1.35 && e > d * 1.18 && e <= 900 && Math.abs(a - e) / Math.max(a, e) <= .28 && Math.abs(b - d) / Math.max(b, d) <= .3;
  }

  function matchHeart(v) {
    return v.length === 6 && isShort([v[0], v[1], v[4], v[5]], 65, 235, 1.65) && isPause([v[2], v[3]], 440, 790, 1.35);
  }

  function matchEcho(v) {
    return v.length === 7 && isShort([v[0], v[2], v[4], v[6]], 65, 235, 1.6) && isPause([v[1], v[3], v[5]], 380, 720, 1.35);
  }

  function matchBlue(v) {
    return v.length === 8 && isShort(v, 50, 175, 1.75) && mean(v) <= 135 && v.reduce((sum, value) => sum + value, 0) <= 1250;
  }

  function matchDawn(v) {
    return v.length === 9 && isPause([v[0], v[2], v[5]], 500, 820, 1.38) && isShort([v[1], v[3], v[4], v[6], v[7], v[8]], 65, 205, 1.85);
  }
  ...
  function matchPure(v) {
    ...
    return isPause(pauses, 560, 790, 1.22) && isShort(shortBeats, 65, 165, 1.45) && mean(shortBeats) <= 130 && mean(pauses) >= mean(shortBeats) * 4.2 && total >= 2400 && total <= 4000;
  }
  ...
  function classifyCommon(values) {
    ...
    if ((ratio >= 2 && Math.min(...sample) <= 330 && Math.max(...sample) >= 520) || variation > .36) return 'forest';
    return 'spark';
  }
  ...
  function makeFlame(type) {
    ...
    return {
      id: `rarity-v2-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      type,
      name: meta[0],
      description: meta[4],
      colors: meta[2],
  ...
  async function postFlame(flame) {
    ...
    try {
      ...
      // Keep the local flame visible; a later retry or poll can recover.
    }
  }
  ...
  function finalize() {
    finalizeTimer = 0;
    if (locked || holdPointerId !== null || taps.length < 3) return;
    generate(classify());
  }
  ...
  function darkCandidate() {
    return taps.length === 2 && taps[1].time - taps[0].time >= 340 && taps[1].time - taps[0].time <= 460;
  }

  function beginDarkHold(event) {
    if (!darkCandidate()) return false;
  ...
  function cancelDarkHold(pointerId) {
    if (holdPointerId === null || (pointerId !== undefined && pointerId !== holdPointerId)) return;
    clearTimeout(holdTimer);
    holdTimer = 0;
    holdPointerId = null;
  ...
  function onPointerDown(event) {
    ...
    taps.push({ time: now, pointerId: event.pointerId });
  ...
  function syncPayload(payload) {
    ...
    // An empty polling response must never clear taps or cancel the finalize timer.
  }
  ...
  button.addEventListener('pointerleave', onPointerUp, true);
